    ...
    <div className="space-y-6">
      {groupedMonths.map((group) => (
        <div key={group.label}>
          ...
          <div className="hidden sm:block rounded-2xl border border-zinc-200 overflow-hidden bg-white shadow-xs">
            <table className="w-full text-xs">
              <thead className="bg-zinc-50/50">
                <tr className="text-[10px] uppercase tracking-wider text-zinc-500 font-bold border-b border-zinc-150">
              ...
              <tbody className="divide-y divide-zinc-150 text-zinc-800">
                {group.bookings.map((booking) => {
                  ...
                  return (
                    <tr
                      key={booking.id}
                      className={`hover:bg-zinc-50/70 transition-colors cursor-pointer ${isActive ? "bg-indigo-50/45" : ""}`}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 font-medium text-zinc-900">{booking.guestName}</td>
                      <td className="px-4 py-3 text-zinc-500">{formatShortDate(booking.checkInDate)}</td>
                      <td className="px-4 py-3 text-zinc-500">{formatShortDate(booking.checkOutDate)}</td>
                      <td className="px-4 py-3 text-center text-zinc-700 font-semibold">{booking.nights}</td>
                      <td className="px-4 py-3 text-center text-zinc-500">{booking.guestsTotal}</td>
                      <td className="px-4 py-3 text-right text-zinc-850 font-bold">{booking.totalPrice}€</td>
            ...
            {group.bookings.map((booking) => {
              ...
              return (
                <div
                  key={booking.id}
                  onClick={() => onSelectBooking?.(booking)}
                  className={`p-3 rounded-xl border space-y-2 cursor-pointer transition-all hover:border-indigo-500/50 hover:shadow-xs ${
                    isActive
                ...
                >
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-zinc-150">
                    <span className="text-[10px] text-zinc-500">
                      Monto: <strong className="text-zinc-850 font-bold">{booking.totalPrice}€</strong>
                    </span>
                    <div>
                      {booking.fullyPaid ? (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-emerald-50 border border-emerald-250 text-emerald-700">
                          Pagado
                        </span>
                      ) : booking.depositPaid ? (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-blue-50 border border-blue-250 text-blue-700">
                          Señal Pagada
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold bg-amber-50 border border-amber-250 text-amber-700">
                          Pendiente
                        </span>
                      )}
                    </div>
